...
class ValuationGapMultiplesOptionsStrategy(OptionsStrategy):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            ...
            tenor_days=tenor_days, gap_basis=gap_basis, multiples_only=multiples_only, **kwargs,
        )
    ...
    def _candidates(self, signals: pd.DataFrame) -> pd.DataFrame:
        ...
        df = signals
        if self.multiples_only and "source" in df.columns:
        ...
        if df.empty:
            return df.assign(_gap=[], _abs_gap=[])
